...
def read_split_data(root: str, val_rate: float = 0.2):
    ...
    #返回数据下的文件名，也即每种花的名字即标签 ，其中os.path.isdir用于检查是否是一个目录即文件夹
    # 排序，保证各平台顺序一致  
    class_names.sort()
    # 生成类别名称以及对应的数字索引
    class_indices = dict((k, v) for v, k in enumerate(class_names))
    #enumerate 函数用来返回每个可迭代对象的索引和元素
    json_str = json.dumps(dict((val, key) for key, val in class_indices.items()), indent=4)
    with open('class_indices.json', 'w') as json_file:
        json_file.write(json_str)
    
    #将所有花的类名也即标签存在字典中，并把它存到json文件中
    #这边有个处理即反向储存，字典的形式为：{'花1':0,'花2':1} 相当于把类名作键，并被赋予了整数值表示类别，方便后续进行匹配标签分类
    ...
    for cla in class_names:
        ...
        #os.path.splitext（path)[-1]可以取出扩展名，一般用来判断某个文件是否含有相应的扩展名
        # 排序，保证各平台顺序一致
        images.sort()
        # 获取该类别对应的索引
        ...
        #random.sample返回一个列表，是从列表中随机采取某些个数的结果
